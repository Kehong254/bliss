...
describe('LifeScreen Tests', () => {

    it('renders the first question and handles answer submission correctly', async () => {
      const mockNavigate = jest.fn();
      const { getByTestId, getByPlaceholderText, getByText,queryByText } = render(<LifeScreen navigation={{ navigate: mockNavigate }} />);

  
      // 由于问题现在是以图片形式展示，我们需要检查问题的图片是否正确渲染
      expect(getByTestId("image-q2-1")).toBeTruthy();
